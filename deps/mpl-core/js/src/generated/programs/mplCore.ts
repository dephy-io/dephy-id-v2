...
export function identifyMplCoreAccount(
  account: { data: ReadonlyUint8Array } | ReadonlyUint8Array
): MplCoreAccount {
  const data = 'data' in account ? account.data : account;
  if (containsBytes(data, getKeyEncoder().encode(Key.AssetV1), 0)) {
    return MplCoreAccount.AssetV1;
  }
  if (containsBytes(data, getKeyEncoder().encode(Key.CollectionV1), 0)) {
    return MplCoreAccount.CollectionV1;
  }
  throw new Error(
    'The provided account could not be identified as a mplCore account.'
  );
}
