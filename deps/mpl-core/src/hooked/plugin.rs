...
pub fn fetch_plugins(account_data: &[u8]) -> Result<Vec<RegistryRecord>, std::io::Error> {
    ...
    let filtered_plugin_registry = plugin_registry
        ...
        .iter()
        .filter_map(|record| {
            PluginType::try_from_slice(&[record.plugin_type]).ok().map(|plugin_type| RegistryRecord {
                plugin_type,
...
pub fn list_plugins(account_data: &[u8]) -> Result<Vec<PluginType>, std::io::Error> {
    ...
    Ok(plugin_registry
        .registry
        .iter()
        .filter_map(|registry_record| PluginType::try_from_slice(&[registry_record.plugin_type]).ok())
        .collect())
}
